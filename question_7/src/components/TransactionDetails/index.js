...
const TransactionDetails = (props) => {
  const renderRecipientDetails = (recipient) => {
    return Object.keys(recipient).map((key, index) => {
      ...
      return (
        <li key={`transaction-details-${index}`}>
          <p>{ key } :</p>
          <ul>
            { Object.keys(recipient[key]).map((subKey, subIndex) => {
              if (typeof recipient[key][subKey] === 'string') {
                return (<li key={`transaction-subdetails-${subIndex}`}>{`${subKey} : ${recipient[key][subKey]}`}</li>)
              }
              return null;
            })}
          </ul>
